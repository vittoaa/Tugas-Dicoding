...
years = changping_df['year'].unique()
selected_year = st.selectbox("Pilih Tahun Visualisasi yang diinginkan untuk Tren Musiman", options=np.append(["Semua"], years))

# **Filter Data Berdasarkan Tahun yang Dipilih**
if selected_year != "Semua":
    changping_df = changping_df[changping_df['year'] == int(selected_year)]

st.subheader("Pilih jika ingin melihat detail perihal data Kota Changping")

# Pastikan data tidak kosong setelah filtering
if not changping_df.empty:
    ...
    st.write("""
    - **Tren Musiman Konsentrasi SO₂ dan NO₂:** Setiap tahun, konsentrasi polutan cenderung menurun pada kuartal ke-3, diikuti oleh lonjakan signifikan setelahnya. Pola ini mungkin disebabkan oleh faktor cuaca dan aktivitas manusia.
    - **Korelasi antara Suhu (TEMP) dan Konsentrasi O₃:** Korelasi positif menunjukkan bahwa ketika suhu meningkat, konsentrasi O₃ juga cenderung meningkat.
    """)
else:
